...
export const getGallery = async (phoneNumber, cb) => {
  try {
    const querySnapshot = await getDocs(
      query(collection(db, "gallery"), where("businessId", "==", phoneNumber)),
    );
    const data = querySnapshot.docs.map((doc) => doc.data());
    cb(data);
  } catch (e) {
  ...
  try {
    const querySnapshot = await getDocs(
      query(
        ...
        where("companyOwner", "==", phoneNumber),
      ),
    );
    const data = querySnapshot.docs.map((doc) => doc.data());
    cb(data);
...
export const getCarData = async (Key_Ref, cb) => {
  try {
    const querySnapshot = await getDocs(
      query(collection(db, "vehicles"), where("regNumber", "==", Key_Ref)),
    );
    const data = querySnapshot.docs.map((doc) => doc.data());
    cb(data);
  } catch (e) {
...
export const getMessages = async (user, cb) => {
  try {
    const unsubscribe = onSnapshot(
      query(collection(db, "chats"), where("uniqueUser", "==", user)),
      (querySnapshot) => {
        const messagesFireStore = querySnapshot
          .docChanges()
